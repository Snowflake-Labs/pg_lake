"""External-reader check for the file_sequence_number fix in PR 300.

Not part of the PR, which ships no test for it. Two commits in
metadata_operations.c and write_manifest.c started populating a manifest
entry's file_sequence_number (it was hard-coded null/absent) and stopped
writing sequence_number into that field. Nothing in the suite reads the field
back, so this reads it two ways: straight out of the Avro with fastavro, and
through pyiceberg, which is the reader an outside consumer would use.
"""
...
@pytest.mark.location_prefix(f"s3://{TEST_BUCKET}/pr300_file_seq")
