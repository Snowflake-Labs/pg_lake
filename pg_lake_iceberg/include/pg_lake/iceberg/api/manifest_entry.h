...
/*
 * Predicate function to filter manifest entries.
 *
 * The predicate function should return true if the manifest entry should be included in the result.
...
const char *IcebergAvroLogicalTypeName(IcebergScalarAvroType type);
const char *IcebergAvroTypeSchemaString(IcebergScalarAvroType type);
