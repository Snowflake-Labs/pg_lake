 ...
 * client thread pool and runs on its own OS thread.  The default therefore has
 * to be a number of threads a host can actually carry: at 10000 the process
 * ran into the kernel's thread limit long before the cap engaged, so it never
 * protected anything.  1000 leaves plenty of room above a Postgres
 * max_connections of 500 (a backend can hold more than one connection, e.g.
 * one per parallel worker) plus the cancellations that share the same count.
 */
