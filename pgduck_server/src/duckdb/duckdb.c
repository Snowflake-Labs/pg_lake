				   ...
				   char *initFilePath)
{
	 ...
	 * is registered by the aws extension, not httpfs, for types
	 * {s3, r2, gcs, aws}. It resolves the *AWS* credential chain, so the
	 * secret ended up holding an AWS key scoped to gcs:// and gs://, every
	 * GCS request got SigV4-signed with it, and Google returned 403. It also
	 * broke reads of public buckets, because a non-empty secret makes httpfs
	 * sign instead of sending an unauthenticated request.
	 *
