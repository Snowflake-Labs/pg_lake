...
def test_pg_lake_manage_cache_disk_pressure(s3, pgduck_conn):
    ...
    pg_lake_engine.max_cache_size is a configured number, and its default of
    20 GiB is larger than the volume a lot of deployments give the cache. The
    cache then never reaches its budget: it grows until the volume is full, and
    from that point on every write into the cache fails with ENOSPC while cache
    management reports a cache that is comfortably inside its budget.
    ...
    # A byte budget the cache never comes close to, like the default
    cache_size = 20 * 1024 * 1024 * 1024
