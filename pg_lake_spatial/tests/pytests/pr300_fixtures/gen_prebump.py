...
so the PR build reads them as an older writer's output.

    python3 -m venv /tmp/duckdb143venv
    /tmp/duckdb143venv/bin/pip install 'duckdb==1.4.3'
    /tmp/duckdb143venv/bin/python gen_prebump.py

The same interpreter is what PR300_DUCKDB143_PYTHON should point at when running
test_pr300_e2e.py.
"""
...
) TO '{out}/prebump_geo.parquet' (FORMAT parquet)
"""
)
...
) TO '{out}/prebump_wkb.parquet' (FORMAT parquet)
"""
)
...
) TO '{out}/prebump_decimal.parquet' (FORMAT parquet)
"""
)

for f in ["prebump_geo.parquet", "prebump_wkb.parquet", "prebump_decimal.parquet"]:
    print(f, os.path.getsize(f"{out}/{f}"), "bytes")

meta = c.execute(
    f"SELECT key, value FROM parquet_kv_metadata('{out}/prebump_geo.parquet')"
).fetchall()
