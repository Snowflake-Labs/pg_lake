...
GDALReadFunctionCall's keep_wkb argument is passed two ways:
...
just at CREATE FOREIGN TABLE instead of at scan time. The other GDAL tests
...
def test_gdal_curve_geometry_describe(
    ...
    error = run_command(
        f"""
        CREATE SCHEMA gdal_curve;
        CREATE FOREIGN TABLE gdal_curve.fdw ()
    ...
    cols = {
        ...
        for c in run_query(
            ...
            "WHERE attrelid = 'gdal_curve.fdw'::regclass "
    ...
    rows = run_query(
        "SELECT ST_GeometryType(geometryproperty) AS t FROM gdal_curve.fdw",
        user_conn,
    )
