"""Extra end-to-end checks for the DuckDB 1.5.5 bump (PR 300).

Not part of the PR. These target claims the PR makes that its own tests do not
assert, and paths a version bump can silently change.
"""
...
FIXTURES = os.path.join(os.path.dirname(__file__), "pr300_fixtures")
...
# pr300_fixtures/gen_prebump.py for how to make one.
DUCKDB143_PYTHON = os.environ.get("PR300_DUCKDB143_PYTHON")
...
def prebump_files(s3):
    """Uploads files written by DuckDB 1.4.3, the pin before this PR."""
    for name in (
        "prebump_geo.parquet",
        "prebump_wkb.parquet",
        "prebump_decimal.parquet",
    ):
        s3.upload_file(f"{FIXTURES}/{name}", TEST_BUCKET, f"pr300/{name}")
    yield


def test_prebump_geoparquet_reads_after_bump(
    user_conn, spatial_analytics_extension, prebump_files
):
    """A GeoParquet file written by the old DuckDB still reads, as geometry."""
    url = f"s3://{TEST_BUCKET}/pr300/prebump_geo.parquet"
    run_command(
        f"CREATE FOREIGN TABLE pr300_prebump_geo () SERVER pg_lake OPTIONS (path '{url}');",
        user_conn,
    )
    ...
    cols = run_query(
        ...
        WHERE attrelid = 'pr300_prebump_geo'::regclass AND attnum > 0 AND NOT attisdropped
        ORDER BY attnum
        """,
        user_conn,
    )
    ...
    rows = run_query(
        "SELECT id, ST_AsText(geom) AS wkt FROM pr300_prebump_geo ORDER BY id",
        user_conn,
    )
...
def test_prebump_wkb_blob_reads_after_bump(
    user_conn, spatial_analytics_extension, prebump_files
):
    """The plain-WKB shape pg_lake itself writes still reads as geometry."""
    url = f"s3://{TEST_BUCKET}/pr300/prebump_wkb.parquet"
    run_command(
        f"""
        CREATE FOREIGN TABLE pr300_prebump_wkb (id int, geom geometry)
        SERVER pg_lake OPTIONS (path '{url}');
        """,
        user_conn,
    )
    rows = run_query(
        "SELECT id, ST_AsText(geom) AS wkt FROM pr300_prebump_wkb ORDER BY id",
        user_conn,
    )
    assert [wkt(r["wkt"]) for r in rows] == [
        wkt("POINT(3 4)"),
...
@pytest.mark.skipif(
    ...
    reason="set PR300_DUCKDB143_PYTHON to an interpreter with duckdb 1.4.3",
)
def test_geometry_write_bytes_and_footer(
    ...
    covered.
    """
    url = f"s3://{TEST_BUCKET}/pr300/written_geo.parquet"
    run_command(
        f"""
        COPY (
    ...
    s3.download_file(TEST_BUCKET, "pr300/written_geo.parquet", local)
...
def test_geometry_round_trip_srid_preserved(user_conn, spatial_analytics_extension, s3):
    ...
    url = f"s3://{TEST_BUCKET}/pr300/srid_roundtrip.parquet"
    run_command(
        f"""
        COPY (SELECT 1 AS id, 'SRID=4326;POINT(3 4)'::geometry AS geom)
        TO '{url}';
        CREATE FOREIGN TABLE pr300_srid (id int, geom geometry(Point, 4326))
        SERVER pg_lake OPTIONS (path '{url}');
        """,
        user_conn,
    )
    rows = run_query(
        "SELECT ST_SRID(geom) AS srid, ST_AsText(geom) AS wkt FROM pr300_srid",
        user_conn,
    )
...
def test_decimal38_stats_after_dropping_return_stats_patch(
    user_conn, extension, prebump_files
):
    ...
    """
    url = f"s3://{TEST_BUCKET}/pr300/prebump_decimal.parquet"
    run_command(
        f"CREATE FOREIGN TABLE pr300_dec () SERVER pg_lake OPTIONS (path '{url}');",
        user_conn,
    )
    ...
    zero = "0::numeric(38,0)"
    assert run_query("SELECT count(*) AS c FROM pr300_dec", user_conn)[0]["c"] == 3
    ...
    for predicate, expected in [
        ...
        got = [
            r["id"]
            for r in run_query(
                f"SELECT id FROM pr300_dec WHERE {predicate} ORDER BY id", user_conn
            )
        ]
...
def test_written_decimal38_stats_roundtrip(user_conn, extension, s3):
    ...
    url = f"s3://{TEST_BUCKET}/pr300/written_decimal.parquet"
    pos = "99999999999999999999999999999999999999::numeric(38,0)"
    neg = "(-99999999999999999999999999999999999999)::numeric(38,0)"
    zero = "0::numeric(38,0)"
    run_command(
        ...
        CREATE FOREIGN TABLE pr300_wdec () SERVER pg_lake OPTIONS (path '{url}');
        """,
        user_conn,
    )
    for predicate, expected in [
        ...
        got = [
            r["id"]
            for r in run_query(
                f"SELECT id FROM pr300_wdec WHERE {predicate} ORDER BY id", user_conn
            )
        ]
        assert got == expected, f"{predicate} -> {got}, want {expected}"
    user_conn.rollback()
...
@pytest.mark.location_prefix(f"s3://{TEST_BUCKET}/pr300_evo")
def test_added_column_filter_after_schema_evolution(
    ...
    run_command(
        """
        CREATE TABLE pr300_evo (id int) USING iceberg;
        INSERT INTO pr300_evo VALUES (1), (2);
        """,
        pg_conn,
    )
    pg_conn.commit()
    ...
    run_command(
        """
        ALTER TABLE pr300_evo ADD COLUMN added_int bigint;
        ALTER TABLE pr300_evo ADD COLUMN added_text text;
        INSERT INTO pr300_evo VALUES (3, 30, 'c');
        """,
        pg_conn,
    )
    pg_conn.commit()

    assert run_query("SELECT count(*) AS c FROM pr300_evo", pg_conn)[0]["c"] == 3
    got = [
        r["id"]
        for r in run_query(
            "SELECT id FROM pr300_evo WHERE added_int IS NULL ORDER BY id", pg_conn
        )
    ]
    assert got == [1, 2]
    got = [
        r["id"]
        for r in run_query(
            "SELECT id FROM pr300_evo WHERE added_int = 30 ORDER BY id", pg_conn
        )
    ]
    assert got == [3]
    got = [
        r["id"]
        for r in run_query(
            "SELECT id FROM pr300_evo WHERE added_text = 'c' ORDER BY id", pg_conn
        )
    ]
    assert got == [3]
    run_command("DROP TABLE pr300_evo", pg_conn)
    pg_conn.commit()
...
def test_literal_glob_chars_in_path(user_conn, extension, s3):
    ...
    key = "pr300/specialChars!@#$%^&*()_+/data.csv"
    s3.put_object(Bucket=TEST_BUCKET, Key=key, Body=b"id,name\n1,a\n2,b\n")

    url = f"s3://{TEST_BUCKET}/{key}"
    run_command(
        f"""
        CREATE FOREIGN TABLE pr300_special (id int, name text)
        SERVER pg_lake OPTIONS (path '{url}', format 'csv', header 'true');
        """,
        user_conn,
    )
    rows = run_query("SELECT id, name FROM pr300_special ORDER BY id", user_conn)
...
def test_glob_still_expands_with_special_chars_present(user_conn, extension, s3):
    ...
    for i in (1, 2):
        s3.put_object(
            Bucket=TEST_BUCKET,
            Key=f"pr300/globdir/part{i}.csv",
            Body=f"id,name\n{i},n{i}\n".encode(),
        )
    url = f"s3://{TEST_BUCKET}/pr300/globdir/*.csv"
    run_command(
        f"""
        CREATE FOREIGN TABLE pr300_glob (id int, name text)
        SERVER pg_lake OPTIONS (path '{url}', format 'csv', header 'true');
        """,
        user_conn,
    )
    rows = run_query("SELECT id FROM pr300_glob ORDER BY id", user_conn)
...
def test_url_encoded_prefix_partition(user_conn, extension, s3):
    ...
    s3.put_object(
        Bucket=TEST_BUCKET, Key="pr300/spacedir/a b/data.csv", Body=b"id\n7\n"
    )
    url = f"s3://{TEST_BUCKET}/pr300/spacedir/**/*.csv"
    run_command(
        f"""CREATE FOREIGN TABLE pr300_space (id int) SERVER pg_lake
            OPTIONS (path '{url}', format 'csv', header 'true');""",
        user_conn,
    )
    rows = run_query("SELECT id FROM pr300_space", user_conn)
...
def test_missing_gdal_layer_errors_and_server_survives(
    ...
    s3.put_object(Bucket=TEST_BUCKET, Key="pr300/layers.geojson", Body=geojson)
    url = f"s3://{TEST_BUCKET}/pr300/layers.geojson"

    error = run_command(
        f"""
        CREATE FOREIGN TABLE pr300_badlayer () SERVER pg_lake
...
def test_gdal_geometry_reads_and_is_not_shipped(
    ...
    s3.put_object(Bucket=TEST_BUCKET, Key="pr300/gdal_pt.geojson", Body=geojson)
    url = f"s3://{TEST_BUCKET}/pr300/gdal_pt.geojson"

    run_command(
        f"""
        CREATE FOREIGN TABLE pr300_gdal () SERVER pg_lake
    ...
    cols = dict(
        ...
        for r in run_query(
            ...
            WHERE attrelid = 'pr300_gdal'::regclass AND attnum > 0 AND NOT attisdropped
            """,
            user_conn,
        )
    )
    ...
    rows = run_query("SELECT ST_AsText(geom) AS wkt FROM pr300_gdal", user_conn)
    assert wkt(rows[0]["wkt"]) == wkt("POINT(3 4)")

    # a spatial function over the GDAL geometry evaluates locally, and is right
    rows = run_query(
        "SELECT ST_X(geom) AS x, ST_Y(geom) AS y FROM pr300_gdal", user_conn
    )
