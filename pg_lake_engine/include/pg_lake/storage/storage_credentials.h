 ...
 *
 * PROTOTYPE (Onder's redesign): a storage-credential resolver that
 * lives at the bottom of the include stack (pg_lake_engine) and is fed
 * by a provider hook installed by pg_lake_iceberg.  This inverts the
 * engine <- iceberg dependency so any storage-touching code in the
 * engine (deletion queue, remote storage) can resolve credentials.
 *
 * Unlike the previous "push from a per-backend cache at executor entry"
 * model, this is a pull: EnsureStorageCredentials is called where a
 * storage path is about to be touched, resolves the credentials on
 * demand (issuing a REST loadTable on a miss via the provider), and
 * reconciles the pgduck_server secret set -- (re)pushing fresh
 * credentials and dropping secrets the provider no longer returns.
 *
 * Known limitations (inherent to a shared pgduck_server with global,
 * in-memory secrets; called out here for the design discussion):
 *
 *  1. pgduck_server restart.  This backend tracks which secrets it
 *     pushed and skips a re-push while the cached expiry is still safe.
 *     If pgduck_server restarts, those secrets are gone but the backend
 *     still believes them present, so scans can 403 until the next
 *     reconcile that has other work -- bounded by the credential TTL
 *     (<=1h for STS).  A full fix needs either a pgduck boot-id handshake
 *     (free on connect) or catching the S3 403 to force a re-push; both
 *     are out of scope for the prototype.
 *
 ...
 *     prefix) are ambiguous to DuckDB's tie-break.  This is a property of
 *     sharing one DuckDB instance with global temp secrets, not a
 *     regression from the old model.
 */
 ...
 * relation.  Call it after storage cleanup when a table is dropped.
 */
