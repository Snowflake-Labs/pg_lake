					...
					List *csvOptions)
{
  ...
  if (columnsMap != NULL)
  {
     ...
     * With auto_detect on (the default), DuckDB activates a string-to-LIST
     * cast path that can segfault when a multidimensional-array value is
     * followed by NULL rows (uninitialized offset/length in nullify_parent).
     * The types are already explicit in columnsMap, so the sniffer adds no
     * value and must not run.  header= is always explicit via the caller's
     * csvOptions, so header detection is unaffected.
     * See: https://github.com/duckdb/duckdb/issues/23373
     */
