	...
	PG_TRY();
	{
		...
		if (useReturnStats && dataFileStats != NULL)
		{
			...
			*dataFileStats = GetDataFileStatsListFromPGResult(result, leafFields, schema);
			...
			foreach(lc, *dataFileStats)
			{
				DataFileStats *stats = (DataFileStats *) lfirst(lc);
...
ParseDuckdbColumnMinMaxFromText(const char *input, List **names, List **mins, List **maxs)
{
	...
	while (*ptr != '\0')
	{
		...
		*names = lappend(*names, columnName);
		...
		{
			ereport(ERROR,
					(errmsg("invalid duckdb column min/max format: missing min or max value for column %s", columnName)));
		}
