...
OpenCopyInStreamToPGDuck(PGDuckConnection * pgDuckConnection, const char *queryString)
{
	 ...
	 * process_query_message via the 'Q' message type. PQsendQueryParams
	 * uses the extended protocol which goes through Parse/Bind/Execute and
	 * doesn't currently support our RECEIVE prefix.
	 */
	 ...
	 * waiting on pgduck. Run #2v hung 4 minutes in this exact state because of
	 * that. WaitForResult uses the standard pgduck-client wait pattern
...
FinishCopyInStreamToPGDuck(PGDuckConnection * pgDuckConnection)
{
	 ...
	 * iteration so a single call here is sufficient — but loop defensively in
	 * case pgduck sends additional results (e.g. CommandComplete + Z).
	 */
