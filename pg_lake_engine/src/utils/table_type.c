...
{
	if (IsInternalIcebergTable(foreignTableId))
		return true;
	...
	return (writableOption != NULL ? defGetBoolean(writableOption) : false);
}
...
/*
 * IsInternalIcebergTable - check if the table is an internal iceberg table (written by us).
 */
bool
...
/*
 * IsExternalIcebergTable - check if the table is an external iceberg table (written by others).
 */
bool
