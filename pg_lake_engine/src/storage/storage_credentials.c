 ...
 *
 * PROTOTYPE (Onder's redesign) storage-credential resolver.  See
 * storage_credentials.h for the rationale.  This replaces the old
 * "push from a per-backend cache at executor entry" flow in
 * pg_lake_table/src/fdw/vended_credentials.c with a pull driven by a
 * provider hook.
 */
 ...
 * CopyErrorData() and the DEBUG1 elog below assert they do not run in
 * ErrorContext.
 */
static void
SwallowBestEffortError(const char *what)
{
	ErrorData  *edata;
	...
	elog(DEBUG1, "storage-credential resolver: ignoring %s failure: %s",
		 what, edata->message);
...
ResolveStorageCredentials(Oid relationId)
{
	...
	PG_CATCH();
	{
		...
		SwallowBestEffortError("credential resolution");
		creds = NIL;
	}
	PG_END_TRY();
...
MakeVendedS3Secret(const StorageCredential * sc)
{
	...
	secret.useSsl = NULL;		/* inherited from the bucket secret */
...
ReconcileSecretsOnConnection(Oid relationId, List *toPush, List *toDrop)
{
	...
	PG_TRY();
	{
		...
		foreach(lc, toPush)
		{
			...
			entry = hash_search(PushedSecrets, key, HASH_ENTER, &found);
			if (!found)
			{
				MemoryContext old = MemoryContextSwitchTo(PushedSecretsContext);

				entry->secretKey = pstrdup(sc->secretKey);
...
BestEffortReconcile(Oid relationId, List *toPush, List *toDrop)
{
	...
	PG_CATCH();
	{
		/* Leave ErrorContext before inspecting/flushing the error. */
		MemoryContextSwitchTo(callerContext);
		SwallowBestEffortError("secret reconcile");
	}
	PG_END_TRY();
}
