...
- Numeric types are mapped based on precision:
    ...
    - Only the column type is rewritten, and only for a `numeric` or `numeric[]` column. A `numeric` nested in a composite type or a map keeps the type you declared — rewriting it would mean giving the column a composite type you never wrote — but it is still stored as an Iceberg `double`. `\d` therefore shows `numeric` while values beyond the precision of `double precision` lose digits; `CREATE TABLE` reports this with a notice.
