...
- **Intervals** are stored as a struct with `months`, `days`, and `microseconds` fields in Iceberg/Parquet. This is transparent for reads and writes through pg_lake, but external tools that read the Parquet files will see the struct representation rather than an interval type.
- **Multidimensional array values** are not representable in Iceberg. PostgreSQL does not have a separate multidimensional array type — a column declared as `int[]` can hold both `ARRAY[1,2,3]` and `ARRAY[ARRAY[1,2], ARRAY[3,4]]`. Iceberg maps `int[]` to a flat `list`, so only one-dimensional values are supported. Multidimensional values are subject to `out_of_range_values` handling: rejected by default (`error`), or replaced with `NULL` in `clamp` mode. See [Out-of-range value handling](#out-of-range-value-handling).
...
By default the drop enumerates every file the table references (metadata, manifest lists, manifests and data files) so the deletion queue holds exact per-file entries. For a table stored under pg_lake's managed location prefix, each table occupies a unique prefix, so the whole prefix can be removed without reading any Iceberg metadata. Enabling `pg_lake_table.fast_drop_file_cleanup` makes the drop queue the table's location prefix instead of walking its metadata — a large speedup when dropping tables with many files. VACUUM then removes everything under that prefix in a single recursive pass. On object stores that support bulk deletion (S3), the objects under the prefix are removed in batched requests (up to 1000 keys per request) rather than one request per file, so VACUUM stays fast even for tables backed by very many files. This applies only to managed-location tables; a table created with a custom `location` (which may be shared with other tables) always falls back to per-file enumeration so nothing outside the table is deleted.
