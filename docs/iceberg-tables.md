...
- **`time with time zone` values lose their UTC offset.** Iceberg has no equivalent type, so pg_lake normalizes the value to UTC and stores it as an Iceberg `time`. The point in time is preserved, but the offset it was written with is not: a value inserted as `09:00:00+04` reads back as `05:00:00+00`. PostgreSQL compares `timetz` values including their offset (`'09:00:00+04'::timetz = '05:00:00+00'::timetz` is false), so a query that compares the column to the original literal will not match after the value has been written to Iceberg. The same applies to `timetz` values used as map keys, where the lookup silently returns nothing. Use `timestamptz`, or keep the offset in a separate column, if the offset matters.
