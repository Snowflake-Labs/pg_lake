					  ...
					  Node *whereClause, uint64 *rowsProcessed)
{
	...
	/*
	 * Compute doCopyPushdown up front so the STDIN block can choose
	 * between the streaming-write path (when GUC is on AND we'll push
	 * the COPY down to pgduck) and the file-based path (everything
	 * else). For non-pushdown COPYs the COPY query is wrapped in
	 * TRANSMIT and rows round-trip back through PG; that's a different
	 * shape than the deferred-INSERT we use for streaming, so streaming
	 * + non-pushdown still uses the file-based path. Document the gap.
	 */
	...
	if (IsCopyFromStdin(copyStmt))
	{
		if (useStreamingStdin)
		{
			 ...
			 * pgduck_server RECEIVE sink-path placeholder; the deferred
			 * COPY query that AddQueryResultToTableStream builds will
			 * pick this up and substitute it with the server-local sink
			 * path before running read_csv() on it.
			 */
