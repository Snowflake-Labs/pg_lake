						   ...
						   bool includeChildren, bool isResultRelation)
{
	...
	/*
	 * PROTOTYPE (Onder's redesign): single choke point for storage
	 * credentials on the read path.  This runs for every relation whose
	 * files we are about to scan -- both the plain FDW scan path and the
	 * query-pushdown path funnel through CreatePgLakeScanSnapshot -> here.
	 * The resolver no-ops for non-REST tables and (re)pushes / drops vended
	 * secrets for REST-catalog Iceberg tables before the data query runs.
	 */
