...
typedef struct MultiDataFileUploadDestReceiver
{
	 ...
	 * and the CSV stream writer that owns the libpq COPY-IN connection.
	 * Both NULL on the file-based path.
	 */
...
CreateChildDestReceiver(MultiDataFileUploadDestReceiver * self)
{
	...
	if (StreamingWritesEnabled)
	{
		 ...
		 * format, options, schema, ...) and pass those into OpenCSVStreamWriter
		 * so the deferred RECEIVE query targets the same destination
		 * ConvertCSVFileTo would have.
		 */
...
FlushChildDestReceiver(MultiDataFileUploadDestReceiver * self)
{
	...
	{
		/*
		 * Streaming path: the deferred COPY ... TO query has already
		 * written the destination file(s). FinishCSVStreamWriter waits
		 * for it to complete and returns the resulting StatsCollector,
		 * which BuildCSVInsertionModifications turns into the same
	...
	foreach(modificationCell, insertModifications)
	{
		...
		/*
		 * partition is a pointer into childContext, which we are about
		 * to MemoryContextReset() at the bottom of this function. Any
		 * downstream consumer of self->modifications (notably
		 * ApplyDataFileModifications during PRE_COMMIT, in another
		 * transaction's worth of execution) will dereference it.
		 * Deep-copy under parentContext so the pointer remains valid.
		 *
		 * NULL guard: CopyPartition unconditionally dereferences
		 * partition->fields_length and segfaults on NULL. Unpartitioned
		 * tables produce NULL modification->partition; preserve that
		 * as NULL in the copy.
		 */
