 ...
 *
 * deparse.c
 *		  Query deparser for pg_lake_table
 *
...
#define ADD_REL_QUALIFIER(buf, uniqueId)	\
		appendStringInfo((buf), "%s%d.", REL_ALIAS_PREFIX, (uniqueId))
...
static void deparseColumnRef(StringInfo buf, int varno, int varattno,
							 ...
							 bool varCanBeNullifiedViaOuterJoin);
...
static Node *deparseSortGroupClause(Index ref, List *tlist, bool force_colno,
									deparse_expr_cxt *context);
static void deparseFieldSelect(FieldSelect *node, deparse_expr_cxt *context);
				...
				Expr *expr)
{
	 ...
	 *
	 * Note: we modified this check to volatile functions because some
	 * important time-related functions are stable and we will hand-select
	 * which functions we deem safe.
	 */
					...
					foreign_loc_cxt *case_arg_cxt)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_Const:
			{
				...
				{
						/*
						 * We do not pushdown any reg* types to the remote
						 * server.
						 */
					case REGPROCOID:
		...
		case T_FuncExpr:
			{
				...
				/*
				 * We mark unnest as shippable, but the handling in the FDW is
				 * improper. For instance, if it appears in the ORDER BY of a
				 * window function, then we will push down a regular ORDER BY
				 * query, which causes incorrect results.
				 */
				if (fe->funcid == F_UNNEST_ANYARRAY)
					return false;
		...
		case T_DistinctExpr:	/* struct-equivalent to OpExpr */
			{
				 ...
				 * Var, it can't be sent to remote.
				 */
				if (oe->inputcollid == InvalidOid || oe->inputcollid == DEFAULT_COLLATION_OID)
					 /* OK, inputs are all noncollatable */ ;
				else if (inner_cxt.state != FDW_COLLATE_SAFE ||
				...
				if (agg->aggorder)
				{
					foreach(lc, agg->aggorder)
					{
						...
						if (srt->sortop != typentry->lt_opr &&
							...
							!is_shippable(srt->sortop, OperatorRelationId, NULL))
							return false;
					}
				}
		...
		case T_FieldSelect:
			{
				/*
				 * The conditions here are if we recognize the typid as a
				 * system type, or one that exists in the lake_struct schema
				 * then we support pushdown, otherwise not.
				 */
				FieldSelect *fieldSelect = (FieldSelect *) node;

				if (fieldSelect->resultcollid == InvalidOid)
				{
					/* no collation */
					state = FDW_COLLATE_SAFE;
					collation = DEFAULT_COLLATION_OID;
				...
				{
					state = FDW_COLLATE_SAFE;
					collation = fieldSelect->resultcollid;
				}

				if (!foreign_expr_walker((Node *) fieldSelect->arg, glob_cxt, &inner_cxt, case_arg_cxt))
					return false;
			}
			break;
		case T_CoerceViaIO:
			{
				 ...
				 * an input foreign Var (same logic as for RelabelType).
				 */
				collation = coerceExpr->resultcollid;
				if (collation == InvalidOid)
					state = FDW_COLLATE_NONE;
				else if (inner_cxt.state == FDW_COLLATE_SAFE &&
						 collation == inner_cxt.collation)
		...
		case T_SQLValueFunction:
			return IsShippableSQLValueFunction(((SQLValueFunction *) node)->op);
		default:
	...
	/*
	 * The geography type is not shippable, but we allow geography(geometry)
	 * casts to go through to the rewrite phase where it gets removed. Hence,
	 * we skip the type check for FuncExpr. We already filtered out any
	 * geography-returning FuncExpr other than geography(geometry).
	 */
	if (IsA(node, FuncExpr) && IsGeographyTypeId(exprType(node)))
		check_type = false;

	/*
	 * In some contexts, user-defined types (primarily composites) are allowed
	 * to be pushed down, but some uses (such as casting) are prohibited due
	 * to there being no equivalent behavior on the DuckDB side.
	 */
	 ...
	 */
	if (check_type && !is_shippable(exprType(node), TypeRelationId, node))
		return false;
				   ...
				   PathKey *pathkey)
{
	...
	if (!is_shippable(pathkey->pk_opfamily, OperatorFamilyRelationId, NULL))
		return false;
	...
	*retrieved_attrs = NIL;
	...
	for (i = 1; i <= tupdesc->natts; i++)
	{
						  ...
						  attrs_used))
		{
			...
			deparseColumnRef(buf, rtindex, i, rte, qualify_col, false);
					  ...
					  List **additional_conds, List **params_list)
{
	...
	else
	{
		...
		/*
		 * Always append alias for base relations.
		 */
		appendStringInfo(buf, " %s%d", REL_ALIAS_PREFIX, GetUniqueRelationIdentifier(rte));
...
deparseAnalyzeSql(StringInfo buf, Relation rel,
				  PgLakeSamplingMethod sample_method, double sample_frac,
				  List **retrieved_attrs)
{
 ...
 *
 * varCanBeNullifiedViaOuterJoin is added specifically pg_lake_table,
 * does not exist in postgres_fdw.
 */
static void
deparseColumnRef(StringInfo buf, int varno, int varattno, RangeTblEntry *rte,
				 bool qualify_col, bool varCanBeNullifiedViaOuterJoin)
{
	...
	else if (varattno == 0)
	{
		...
		Relation	rel = table_open(rte->relid, NoLock);;
		bool		first = true;
		StringInfo	whenBuf = makeStringInfo();
		StringInfo	thenBuf = makeStringInfo();
		StringInfo	elseBuf = makeStringInfo();
		 ...
		 * would be true.
		 */
		appendStringInfoString(whenBuf, "CASE WHEN (");
		appendStringInfoString(thenBuf, "THEN ROW (");

		/*
		 * If the whole-row reference is under an outer join then the
		 * returning row must be NULL whenever all the columns are NULL.
		 *
		 * Unfortunately DuckDB does not support count(t1.*)::text kind of
		 * expressions, which postgres_fdw relies on. So, we have to construct
		 * the CASE statement for our purpose.
		 *
		 * If any column is non-NULL, the CASE statement returns the row. It
		 * means that the row is not the result of an unmatched outer join.
		 *
		 * If all columns are NULL, we need to understand whether the row is
		 * result of an unmatched outer join, if so we should preserve as NULL
		 * tuple, not a tuple with all columns are NULL. postgres_fdw does
		 * that by NULL::record, which is also not supported in DuckDB.
		 *
		 * The caller of this function lets us know whether this column
		 * reference is under an outer join. If yes, we can check that the row
		 * is the result of an unmatched outer join. We implement this with
		 * the nullify_any_type function on DuckDB/duckdb_pglake.
		 *
		 * Finally, even if all the columns are NULL and we are not under an
		 * outer join, we can return the row as is.
		 *
		 * Below is an example of the case statement that we are constructing:
		 *
		 * CASE WHEN ((r1.a IS NOT NULL) OR (r1.b IS NOT NULL)) THEN ROW(r1.a,
		 * r1.b) ELSE nullify_any_type(ROW(NULL::integer, NULL::text)) END AS
		 * "case"
		 */
		if (varCanBeNullifiedViaOuterJoin)
			appendStringInfoString(elseBuf, PG_LAKE_INTERNAL_NSP ".nullify_any_type(ROW(");
		else
			appendStringInfoString(elseBuf, "(ROW(");

		TupleDesc	tupdesc = RelationGetDescr(rel);
		int			columnIndex = 1;

		for (columnIndex = 1; columnIndex <= tupdesc->natts; columnIndex++)
		{
			Form_pg_attribute attr = TupleDescAttr(tupdesc, columnIndex - 1);

			/* Ignore dropped attributes. */
			if (attr->attisdropped)
				continue;

			StringInfo	columnRefBuf = makeStringInfo();

			deparseColumnRef(columnRefBuf, varno, columnIndex, rte, qualify_col, varCanBeNullifiedViaOuterJoin);

			if (!first)
			{
				appendStringInfoString(whenBuf, ", ");
				appendStringInfoString(thenBuf, ", ");
				appendStringInfoString(elseBuf, ", ");
				first = false;
			}

			appendStringInfo(whenBuf, "%s  IS NOT NULL", columnRefBuf->data);
			appendStringInfo(thenBuf, "%s", columnRefBuf->data);

			/*
			 * We do not include typemod in the cast, because DuckDB does not
			 * support all forms of typemod that PostgreSQL does. For
			 * instance, it does not support timestamptz(2). For regular
			 * casts, this is handled by not marking the underlying function
			 * as shippable, but here we ignore shippable rules so we need to
			 * be more careful.
			 *
			 * Since we are simply casting NULL, there isn't anything
			 * interesting that the typemod does for us.
			 *
			 * Note that DuckDB does not actually require us adding the casts
			 * at all, but we run this query through the PostgreSQL parser and
			 * ruleutils deparser, and not adding a cast would result in
			 * emitting NULL::unknown, and DuckDB does object to that.
			 */
			appendStringInfo(elseBuf, "NULL::%s",
							 format_type_be(attr->atttypid));

			if (columnIndex != tupdesc->natts)
			{
				appendStringInfoString(whenBuf, " OR ");
				appendStringInfoString(thenBuf, ", ");
				appendStringInfoString(elseBuf, ", ");
			}
			else
		...
		appendStringInfoString(buf, whenBuf->data);
		appendStringInfoString(buf, thenBuf->data);

		appendStringInfo(buf, " ELSE %s END AS \"case\"", elseBuf->data);
...
deparseExpr(Expr *node, deparse_expr_cxt *context)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_Aggref:
			...
			break;
		case T_FieldSelect:
			deparseFieldSelect((FieldSelect *) node, context);
			break;
		case T_CoerceViaIO:
...
deparseVar(Var *node, deparse_expr_cxt *context)
{
	...
	{
		RelOptInfo *varRelOptInfo = context->root->simple_rel_array[node->varno];
		bool		varCanBeNullifiedViaOuterJoin =
			!bms_is_empty(varRelOptInfo->nulling_relids);

		deparseColumnRef(context->buf, node->varno, node->varattno,
						 planner_rt_fetch(node->varno, context->root),
						 qualify_col, varCanBeNullifiedViaOuterJoin);
	}
	else
	{
		/* Treat like a Param */
...
/*
 * Deparse a CoerceViaIO (cast via text) node.
 *
 * The typemod is lost in such a coercion.
 */
static void
deparseCoerceViaIO(CoerceViaIO *node, deparse_expr_cxt *context)
{
	...
	if (node->coerceformat != COERCE_IMPLICIT_CAST)
		appendStringInfo(context->buf, "::%s",
						 deparse_type_name(node->resulttype, -1));
}
 ...
 */
static void
deparseFieldSelect(FieldSelect *fieldSelect, deparse_expr_cxt *context)
{
	appendStringInfoCharMacro(context->buf, '(');
	deparseExpr((Expr *) fieldSelect->arg, context);
	appendStringInfoCharMacro(context->buf, ')');
	appendStringInfoCharMacro(context->buf, '.');

	int			attr_index = fieldSelect->fieldnum;
	char	   *field_name =
		get_attname(get_typ_typrelid(exprType((Node *) fieldSelect->arg)),
					attr_index,
					false);
					...
					deparse_expr_cxt *context)
{
	...
	foreach(lcell, pathkeys)
	{
		...
		if (!OidIsValid(oprid))
		{
			const char *opName = get_opname(oprid);
			const char *opTypeName = format_type_be_qualified(em->em_datatype);
			Oid			opFamilyOid = pathkey->pk_opfamily;

			elog(ERROR, "missing operator %s(%s,%s) in opfamily oid %u",
				 opName, opTypeName, opTypeName, opFamilyOid);
		}
...
/*
 * Deparse (in a fashion) shippable SQLValueFunctions.  While not an explicit
 * deparse, we deparse an equivalent constant based on the op that we have.
 */
static void
deparseSQLValueFunction(SQLValueFunction *node, deparse_expr_cxt *context)
{
	...
	switch (node->op)
	{
		...
		case SVFOP_LOCALTIMESTAMP:
			...
			break;
		default:
			ereport(ERROR, (errmsg("unexpected SQLValueFunction type"),
