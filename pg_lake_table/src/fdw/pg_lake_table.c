...
typedef struct PgLakeFileModifyState
{
	 ...
	 * DestReceiver writing to deleteFile (a local temp under pgsql_tmp).
	 * The file is later read by ApplyDeleteFile via ConvertCSVFileTo or
	 * PerformDeleteFromParquet.
	 *
	 * Streaming-write path (StreamingWritesEnabled=true): deleteDest is
	 * lazily set on the first row received for this source file. Until
	 * then it stays NULL (along with deleteStreamWriter); `deleteFile`
	 * holds the precomputed Parquet position-delete path and the
	 * deleteStream* fields hold the metadata needed to open the stream.
	 * Lazy because we don't want to open a libpq stream / write an empty
	 * Parquet for every source file when only some have matching rows.
	 *
...
typedef struct PgLakeModifyState
{
	 ...
	 * FinishForeignModify runs. The chunk gets re-allocated as a
	 * postgres List by a later lappend in
	 * GetDataFileStatsListFromPGResult, and writer->dest->rDestroy
	 * jumps into garbage → SIGSEGV.
	 *
	 * Mirrors the pattern in
	 * multi_data_file_dest.c::CreateChildDestReceiver, which switches
	 * to its own childContext before the OpenCSVStreamWriter call.
	 *
	 * Created lazily (only when DELETE/UPDATE) by create_foreign_modify
	 * as a sub-context of CurrentMemoryContext at that time, which is
	 * the FDW state's per-statement context — outlives all rows and
	 * survives until FinishForeignModify completes.
	 */
...
DeleteSingleRow(PgLakeModifyState * fmstate, ItemPointer ctid)
{
	...
	/*
	 * Lazy-open the streaming dest receiver on the first row for this
	 * source file. We don't open up front because many source files in
	 * a scan may end up with no matching rows, and an empty stream would
	 * still write an empty Parquet on flush. See create_foreign_modify
	 * for where the stash fields are populated.
	 */
	if (fileModifyState->deleteDest == NULL && StreamingWritesEnabled)
	{
		...
		/*
		 * Allocate the writer + dest + cstate in fmstate->deleteStreamMemoryContext,
		 * not whatever per-tuple context the executor handed us. Without this
		 * switch the chunks get reset / sibling-reused before
		 * FinishForeignModify runs and writer->dest->rDestroy jumps into
		 * garbage. See the field comment in struct PgLakeModifyState.
		 */
				  ...
				  TupleTableSlot *deleteSlot)
{
		...
		fileModifyState->deleteStreamWriter == NULL)
	{
		 ...
		 * deleted, so we don't need a deletion file. Drop the local CSV
		 * temp file to free up disk space and let FinishForeignModify
		 * emit an ADD_DELETION_FILE_FROM_CSV with deleteFile=NULL —
		 * downstream ApplyDeleteFile handles that as "remove the source
		 * file" (liveRowCount==0).
		 *
		 * The streaming path (deleteStreamWriter != NULL) MUST NOT take
		 * this branch: rDestroy mid-stream pfree\'s the DR_copy chunk
		 * and the still-non-NULL deleteStreamWriter then points at freed
		 * memory; FinishForeignModify\'s FinishCSVStreamWriter would hit
		 * a use-after-free + ApplyPrecomputedDeleteFile would receive
		 * a NULL deleteFile.
		 *
		 * For the streaming path, just let WriteDeleteRecord keep
		 * accumulating rows; the resulting parquet position-delete file
		 * will cover all rows in the source, which is semantically
		 * equivalent (iceberg readers see a fully-deleted source via
		 * the position-delete merge).
		 */
...
FinishForeignModify(PgLakeModifyState * fmstate)
{
	...
	foreach(fileModifyCell, fmstate->fileModifyStates)
	{
		...
		if (fileModifyState->deleteStreamWriter != NULL)
		{
			 ...
			 * pgduck's deferred COPY ... TO 'parquet_path' to complete,
			 * and grab the resulting per-file stats. The Parquet position-
			 * delete file already exists on object storage at this point.
			 *
					  ...
					  ModifyTableState *mtstate)
{
	...
	if (operation == CMD_UPDATE || operation == CMD_DELETE)
	{
		 ...
		 * allocations. See the field comment in struct PgLakeModifyState
		 * for the full reasoning. Created as a sub of CurrentMemoryContext
		 * (the FDW state context) so it outlives all per-tuple resets.
		 */
		...
		if (scanState != NULL)
		{
			...
			foreach(fileScanCell, resultTableScan->fileScans)
			{
				...
				else if (StreamingWritesEnabled)
				{
					 ...
					 * position-delete file path now and stash the args
					 * needed to open a CSVStreamWriter targeting it.
					 * The actual stream open is deferred to the first
					 * ExecForeignDelete touching this source file (see
					 * the lazy-open block above WriteDeleteRecord).
					 *
					 * Why deferred: ExecForeignDelete is dispatched per
					 * row based on the row's ctid -> source file mapping.
					 * Many source files in a scan may end up with zero
					 * matching rows, and we don't want to open a libpq
					 * stream (and write an empty Parquet) for those.
					 *
					 * We always go merge-on-read here; copy-on-write is
					 * file-based-only for now (see ApplyPrecomputedDeleteFile).
					 */
					 ...
					 * ApplyDeleteFile's MOR branch does for the
					 * file-based path so a transaction abort still
					 * cleans up partial Parquet writes.
					 */
					bool		isPrefix = false;
					bool		isIcebergTable =
					...
					/*
					 * deleteDest stays NULL until the lazy-open block
					 * fires. WriteDeleteRecord guards on this.
					 */
				}
				else
				{
