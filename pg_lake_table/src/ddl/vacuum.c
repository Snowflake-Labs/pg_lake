						   ...
						   bool isAutoVacuum)
{
	...
	/*
	 * PROTOTYPE (Onder's redesign): resolve storage credentials once for the
	 * whole vacuum.  Compaction, metadata rewrite, deletion-queue drain and
	 * in-progress-file cleanup below all read/write/delete this table's
	 * objects via pgduck_server, and the pushed (temporary) secret is shared
	 * process-wide, so it stays available across the separate
	 * sub-transactions these steps run in.  No-op for non-REST tables.
	 */
