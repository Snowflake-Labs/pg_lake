...
ApplyTrackedIcebergMetadataChanges(bool isVerbose)
{
	...
	/*
	 * Refresh catalog stats before we run the per-table data-file diff
	 * query. The diff joins lake_table.files against the partition-value
	 * and column-stats catalogs; autovacuum does not fire inside an open
	 * transaction, so on workloads that insert tens of thousands of files
	 * in one tx the planner's pg_class.reltuples is wildly stale by the
	 * time GetDataFileMetadataOperations runs. With stale stats the
	 * planner can pick an N^2 nested loop for a query whose result is
	 * comfortably hash-join-able, blowing commit time up by orders of
	 * magnitude.
	 *
	 * Only worth doing if at least one tracked relation actually has
	 * data file changes — DDL-only or noop trackers skip the diff entirely.
	 */
...
{
	static const char *const catalogsToAnalyze[] = {
	...
	for (size_t i = 0; i < lengthof(catalogsToAnalyze); i++)
	{
		resetStringInfo(cmd);
		appendStringInfo(cmd, "ANALYZE %s", catalogsToAnalyze[i]);

		int			spiStatus = SPI_execute(cmd->data, false /* readOnly */ , 0);

		if (spiStatus < 0)
			ereport(ERROR,
					(errmsg("pg_lake: ANALYZE on commit-time catalog failed"),
					 errdetail("SPI_execute(\"%s\") returned %s",
							   cmd->data, SPI_result_code_string(spiStatus))));
	}
