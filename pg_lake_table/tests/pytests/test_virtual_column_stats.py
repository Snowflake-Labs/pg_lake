...
def test_filter_on_virtual_filename_column(s3, pg_conn, extension):
    base = f"s3://{TEST_BUCKET}/virtual_col/"
    ...
    run_command(
        f"""
        CREATE FOREIGN TABLE virtual_col () SERVER pg_lake
    ...
    res = run_query(
        f"SELECT count(*) AS c FROM virtual_col WHERE _filename = '{urls[1]}'", pg_conn
    )
    assert res[0]["c"] == ROWS

    # a value no file has: must prune to nothing rather than read junk stats
    res = run_query(
        f"SELECT count(*) AS c FROM virtual_col WHERE _filename = '{base}nope.parquet'",
        pg_conn,
    )
    assert res[0]["c"] == 0
    ...
    res = run_query(
        f"SELECT count(*) AS c FROM virtual_col WHERE _filename <> '{urls[0]}'", pg_conn
    )
    assert res[0]["c"] == 2 * ROWS

    res = run_query(
        f"SELECT count(*) AS c FROM virtual_col "
    ...
    expected = run_query(
        f"SELECT count(*) AS c FROM virtual_col " f"WHERE id > {2 * ROWS} AND grp = 5",
        pg_conn,
    )
    ...
    res = run_query(
        f"SELECT min(id) AS lo, max(id) AS hi FROM virtual_col "
    ...
    run_command("DROP FOREIGN TABLE virtual_col", pg_conn)
    pg_conn.commit()
